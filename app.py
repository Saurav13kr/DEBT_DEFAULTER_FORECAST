...
def prediction():
    if not session.get("email") is None and not session.get("name") is None:    
        if request.method == "POST":
            ...
            msg = Message('Credit Scorer', sender = 'rajharsh5119@gmail.com', recipients = [str(session['email'])])
            if o[0] >= 0.5:
                msg.body = "Hello "+session['name']+",\n"+"Congratulations!! You are qualified for the debt "
            else:
                msg.body = "Hello "+session['name']+",\n"+"Sorry!! Your are not qualified for debt "
            mail.send(msg)
            session.pop('email')
            session.pop('name')
...
if __name__ == '__main__':
    ...
    app.config['MAIL_USERNAME'] = 'rajharsh5119@gmail.com'
    app.config['MAIL_PASSWORD'] = 'nouvvxvmxdgtymey'
